...
ONE process (run as a LaunchAgent) keeps the Pocket TTS model and voice
states resident in memory, and exposes the `speak` / `list_voices` tools
...
Generation, the cross-session playback lock, mic-suppression and JSONL
logging are all reused unchanged from the library via the `_GENERATOR`
hook — the daemon only swaps the synthesis backend from a cold `uvx`
subprocess to the resident model.
"""
...
from speak_when_done import DEFAULT_LANGUAGE, DEFAULT_VOICE
...
# and plays strictly in arrival order, so audio never overlaps and sessions
...
# states are keyed by (language, voice) so each voice's conditioning prompt is
...
def _enqueue_speak(message: str, voice: str | None) -> dict:
    """Validate + enqueue a speak request; never blocks on synthesis.

    Returns ``queued=True`` and the 1-based ``position`` in the queue.
    """
    if not message or not message.strip():
        return {"success": False, "error": "message is empty"}

    item = _QueuedSpeak(message=message, voice=voice, enqueued_at=time.monotonic())
    with _enqueue_lock:
        try:
            _speak_queue.put_nowait(item)
        except queue.Full:
            logger.error(
                "speak queue full (%d pending); rejecting: %s...",
                _speak_queue.qsize(),
                message[:50],
            )
            return {
...
def _process_item(item: _QueuedSpeak) -> None:
    ...
    if age > STALE_AFTER_S:
        logger.warning(
            "dropping stale queued message (age %.0fs > %.0fs cutoff): %s...",
            age,
            STALE_AFTER_S,
            item.message[:50],
        )
        ...
        return
    result = swd.speak(item.message, voice=item.voice, quiet=True)
    if result.get("success"):
        logger.info("spoken (waited %.1fs in queue)", age)
...
def _worker_loop() -> None:
    ...
    sessions wait their turn in arrival order.
    """
...
def _recent_speaks(limit: int = 40) -> list[dict]:
    ...
    text is OPTIONAL — rows logged before text capture have none. The UI
    renders around whatever is present.
    """
    path = swd._CALL_LOG
    try:
...
<script>
  ...
  async function refresh(){
    try{
      ...
      const hk = JSON.stringify((s.recent||[]).map(r=>[r.ts,r.kind,r.reason]));
...
def speak(message: str, voice: str | None = None) -> dict:
    ...
    """
    logger.info("speak: %s...", message[:50])
    result = _enqueue_speak(message, voice=voice)
    if result.get("success"):
        logger.info("queued at position %s", result.get("position"))
    else:
...
def list_voices() -> dict:
    ...
    return swd.list_voices()


def _preload() -> None:
    """Warm the default-language model and default voice state at boot."""
    try:
        _get_model(DEFAULT_LANGUAGE)
        _get_state(DEFAULT_LANGUAGE, DEFAULT_VOICE)
    except Exception as e:  # noqa: BLE001
